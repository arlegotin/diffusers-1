...
pipe = StableDiffusionControlNetPipeline.from_pretrained(
    "Lykon/dreamshaper-8", controlnet=controlnet, torch_dtype=torch.float16, use_safetensors=True
)
...
pipe = StableDiffusionControlNetPipeline.from_pretrained("Lykon/dreamshaper-8", controlnet=controlnet, use_safetensors=True).to("cuda")
...
Replace the SDXL model with a model like [Lykon/dreamshaper-8](https://huggingface.co/Lykon/dreamshaper-8) to use multiple conditioning inputs with Stable Diffusion models.
