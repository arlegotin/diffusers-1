...
```py
pipeline = AutoPipelineForText2Image.from_pretrained(
	"Lykon/dreamshaper-8", controlnet=controlnet, torch_dtype=torch.float16, variant="fp16"
).to("cuda")
generator = torch.Generator("cuda").manual_seed(31)
...
pipeline = AutoPipelineForText2Image.from_pretrained("Lykon/dreamshaper-8", torch_dtype=torch.float16, variant="fp16").to("cuda")
